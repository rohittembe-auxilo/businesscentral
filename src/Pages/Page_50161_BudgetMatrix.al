page 50161 "Budget Matrix Archive"
{
    Caption = 'Budget Matrix Archive';
    ...
    local procedure SetCommonFilters(var TheGLAccBudgetBuf: Record "G/L Acc. Budget Buffer Archive")
    begin
        with TheGLAccBudgetBuf do begin
            Reset;
            SetRange("Budget Filter", GLBudgetName.Name);
            SetRange("Version No. Filter", GLBudgetName."Version No.");
            if BusUnitFilter <> '' then
    ...
    local procedure SetDimFilters(var TheGLAccBudgetBuf: Record "G/L Acc. Budget Buffer Archive"; LineOrColumn: Option Line,Column)
    var
    ...
    var
        GLBudgetEntry: Record "G/L Budget Entry Archive";
    begin
        GLBudgetEntry.SetRange("Budget Name", GLBudgetName.Name);
        GLBudgetEntry.SetRange("Version No.", GLBudgetName."Version No.");
        ...
        with GLBudgetEntry do
            ...
            then
                SetCurrentKey("Budget Name", "G/L Account No.", "Business Unit Code", "Global Dimension 1 Code", "Version No.")
            else
                SetCurrentKey("Budget Name", "G/L Account No.", Date, "Version No.");
    ...
    local procedure CalcFieldsAndGetBudgetedAmount(var GLAccBudgetBuf: Record "G/L Acc. Budget Buffer Archive") Result: Decimal
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
    ...
    local procedure CalcFieldsAndSetNewBudgetedAmount(var GLAccBudgetBuf: Record "G/L Acc. Budget Buffer Archive"; NewAmount: Decimal)
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterSetCommonFilters(var TheGLAccBudgetBuffer: Record "G/L Acc. Budget Buffer Archive")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterSetDimFilters(var TheGLAccBudgetBuffer: Record "G/L Acc. Budget Buffer Archive"; DimType: Enum "G/L Budget Matrix Dimensions"; DimCodeBuf: Record "Dimension Code Buffer")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCalcFieldsAndGetBudgetedAmount(var GLAccBudgetBuffer: Record "G/L Acc. Budget Buffer Archive"; var Result: Decimal; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeCalcFieldsAndSetNewBudgetedAmount(var GLAccBudgetBuf: Record "G/L Acc. Budget Buffer Archive"; NewAmount: Decimal; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBudgetDrillDownOnBeforePageRun(var GLAccBudgetBuffer: Record "G/L Acc. Budget Buffer Archive"; var GLBudgetEntry: Record "G/L Budget Entry Archive")
    begin
    end;
}
