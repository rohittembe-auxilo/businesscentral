codeunit 50031 "Check Purc Interface Lines"
{
    ...
    begin
        ...
        PLCLogRec.SETRANGE("Interface Type", PLCLogRec."Interface Type"::Purchase);
    ...
    begin
        ...
        IF PLCLogDetail.FINDFIRST THEN BEGIN
            ...
            Rec_PurchHeader1.SETRANGE("Buy-from Vendor No.", RecPurchInterface."Buy-from Vendor No.");//CCIT Vikas 14022020 Added Line For Vendor Invoice no Can be same for different Vendor
            IF NOT Rec_PurchHeader1.FINDFIRST THEN BEGIN
                Rec_PurchHeader.INIT;
                Rec_PurchHeader."Document Type" := Rec_PurchHeader."Document Type"::Invoice;
                ...
                IF Rec_PurchHeader.INSERT(TRUE) THEN BEGIN
                    ...
                    Rec_PurchLine.INIT;
                    Rec_PurchLine."Document Type" := Rec_PurchLine."Document Type"::Invoice;
                    ...
                    //Rec_PurchLine.VALIDATE("Posting Date",Rec_PurchHeader."Posting Date"); //CCIT AN
                    ...
                    ELSE
                       GSTGRTYPE := 0;
                    Rec_PurchLine.VALIDATE("GST Group Type",GSTGRTYPE);*/ //CCIT AN IT takes goods only 30032023
                                                                          //04042023
                    GSTGroup.RESET();
                    GSTGroup.SETRANGE(Code, RecPurchInterface."GST Group Code");
                    IF GSTGroup.FINDFIRST THEN
                        Rec_PurchLine."GST Group Type" := GSTGroup."GST Group Type";

                    Rec_PurchLine.VALIDATE("HSN/SAC Code", RecPurchInterface."HSN/SAC Code");
                    IF Rec_PurchLine."GST Group Code" = '0001' THEN
                        //Rec_PurchLine."GST Base Amount" := 0
                        Rec_PurchLine.FOC := true;
                    // ELSE
                    //     Rec_PurchLine.VALIDATE("GST Base Amount", RecPurchInterface."GST Base Amount");
                    //04042023
                    //Rec_PurchLine.VALIDATE("GST Base Amount",RecPurchInterface."GST Base Amount"); //Commented by CCIT AN
